...
with tab_fcn:
    ...
    st.caption(
        "輸入您持有／考慮買入的 FCN（Fixed Coupon Note，股權連結型定期配息票券）條款，以蒙地"
        "卡羅模擬（幾何布朗運動，波動率／相關性取標的歷史日報酬年化）評估提前出場機率與本金虧損"
        "機率。標的數量大於 1 時視為「worst-of」組合票券：任一標的觸及下限價即視為觸及，須全部"
        "標的都達到提前出場價才會提前出場。模型為簡化估算（固定波動率、月配息、月觀察提前出場），"
