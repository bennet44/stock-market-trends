...
- **FCN風險評估**：針對股權連結型定期配息票券（FCN）做風險試算。輸入標的股票/指數後，以其歷史日報酬計算年化波動率，再用蒙地卡羅模擬（幾何布朗運動）估算不同「下限價 KI%」與「合約期間（3/6/9/12 個月）」組合下的合理年化收益率（風險中性測度，使票券折現價值等於票面）與本金虧損機率（依所選風險評估假設：中性假設或延伸近期歷史走勢）；執行價 Strike%、提前出場 Autocall%、KI 觀察方式（到期日判定／全程觀察）與可接受的本金虧損機率上限皆可調整。分頁會在符合風險容忍度的組合中，挑出年化收益率最高者作為建議參數，並列出各合約期間在風險容忍度內可達到的最高收益率，協助判斷期間長短的取捨。模型僅為簡化估算（單一標的、固定波動率、不含信用風險與手續費），僅供研究參考，非投資建議或實際報價。
