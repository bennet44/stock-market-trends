...
- **設定值自動記憶**：所有分頁的輸入欄位（股票代號、時間範圍、各種參數）會自動存進瀏覽器的 localStorage，下次用同一個瀏覽器打開網頁時會自動帶回上次的設定值；此記憶是依瀏覽器各自儲存（不同使用者/裝置互不影響），且不受 Streamlit Cloud 容器重啟影響。清除瀏覽器資料或換瀏覽器/裝置開啟則會回到預設值。
