...
- **FCN風險評估**：針對股權連結型定期配息票券（FCN）的特定條款做風險評估。可設定「標的數量」（1~5 個）並以逗號分隔輸入對應數量的股票代號；標的數量大於 1 時視為「worst-of」組合票券，任一標的觸及下限價即視為觸及、須全部標的都達到提前出場價才會提前出場。輸入「投資時間（個X月）」「執行價 STRIKE(%)」「提前出場 KO(%)」「下限價 KI(%)」「年化收益率(%)」與 KI 觀察方式（到期日判定／全程觀察）後，以標的近 N 個月（N＝投資時間）歷史日報酬計算年化波動率與（多標的時）相關係數，再用蒙地卡羅模擬（幾何布朗運動／相關常態分配）評估提前出場機率、平均出場月數、本金虧損機率與報酬分布（期望報酬、5% 最差情境報酬），並列出假設無風險利率下的「風險中性參考年化收益率」供與您輸入的年化收益率比較。模型為簡化估算（固定波動率與相關性、不含信用風險與手續費），僅供研究參考，非投資建議或實際報價。
